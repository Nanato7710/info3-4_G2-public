...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(state, "playground/riku/model/Transfer_Learning_best.pth")

    # 学習終了後にスコアをCSV形式で保存
    print("\nTraining complete. Saving all metrics to CSV...")
    df_history = pd.DataFrame(metrics_history)
    df_history.to_csv("playground/riku/model/Transfer_Learning_metrics.csv", index=False)
    print("Done!")
