...
def run_single_seed(config_path: Path, base_config: dict, seed: int, multi_seed: bool) -> dict:
    ...
    # 最初の初期化（ループ内でも更新されますがプレースホルダーとして作成）
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        # 1. モデルの解凍状態を現在のエポック（1始まり）に基づいて更新
        model.unfreeze_stage(epoch)
        
        # 2. 現在の「requires_grad=True」なパラメータ群でオプティマイザーを再ビルド
        optimizer = build_optimizer(model, float(config["learning_rate"]))
        
        # デバッグ用ログ：現在の有効な学習対象パラメータブロック数を表示
        active_params = sum(1 for p in model.parameters() if p.requires_grad)
        print(f"[Debug] Active parameter blocks: {active_params}")
