...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| kazusa-baseline | 主比較 | 0.5 固定 | 0.3498 | 0.0069 |  | 0.3601 | 0.4552 | 0.2365 | 5.6786 |

### 2.1 複数 seed 集計

seed 集計グループ: `template`
...
| seeds | 42, 43, 44 |
| device | auto |
| comparison | {"primary": "kazusa-baseline", "references": []} |
| epochs | 1 |
...
```bash
uv run python experiments/template/run_exp.py
uv run python experiments/template/analyze.py
uv run python experiments/template/make_report.py
```
...
### 6.1 主比較 `kazusa-baseline` とのジャンル別 AP 差分
...
| ファイル | 用途 |
| --- | --- |
| experiments/template/config.yaml | 実験設定 |
| 未生成 | epoch ごとの学習ログ |
| experiments/template/outputs/analysis/overall_model_metrics.csv | validation の全体指標 |
| experiments/template/outputs/analysis/genre_metrics_validation_threshold_0.5.csv | validation のジャンル別指標 |
