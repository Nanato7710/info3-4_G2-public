...
| 新モデル | exp_001 |
| 今回の目的 |  |
| Git branch | exp/riku-Transfer_Learning |
| Git commit |  |
...
| image size | 224 |
| torch.compile | Fales |
| max train samples | 前データ使用 |
| max val samples | 前データ使用 |
| output dir | outputs |
...
|---|---|---|---|---|
| H1 | スコアが低い | 事前学習をやっていないため特徴量を得られきっていない | ディープラーニングの「事前学習」とは？初心者向けにわかりやすく解説　ここからスコアが伸びると考えた | resnet18に事前学習を入れ込む |
| H2 |  |  |  |  |
| H3 |  |  |  |  |
...
- [ ] 前処理
- [ ] augmentation
- [ Resnet18 -> Imagenet事前学習済みResnet18] モデル
- [ ] loss
- [ ] optimizer
- [ ] scheduler
- [ ] threshold
...
```markdown
ResNet18を、ImageNetデータセットで事前学習済みのResNet18へ置き換えた。転移学習を行うため、最終出力層のみを本課題の分類数に合わせて変更し、それ以外の層の重みは固定した。そのため、学習時に更新されるパラメータは出力層のみであり、事前学習によって獲得された特徴抽出能力をそのまま利用した。
```
...
|---|---|---:|---:|---:|---:|---:|---|
| 旧モデル |  |  |  |  |  |  |  |
| ImageNet事前学習ResNet18 | 事前学習追加 | 0.305 | 0.299 | 0.210 | 0.331 |  0.121| fcのみ重み変更可能 |
| 差分 |  |  |  |  |  |  |  |
...
```markdown
今回ImageNetを事前学習させたことによって初めからmarco F1とmAPは0.25を上回っていた。
...
```markdown
どうしても0.4以上を出せなかったため次求める課題として目標0.5付近にまで上げたいと考えている。
