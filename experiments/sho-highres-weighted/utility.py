...
class LetterboxResize:
    """アスペクト比を維持しつつ、不足部分を黒色(0)でパディングして正方形にする変換"""
    ...
    def __call__(self, img):
        ...
        # 縦横比を崩さずに高品質（Bicubic）でリサイズ
        img = img.resize((new_w, new_h), self.interpolation)
        
        # 正方形にするための余白（パディング）を計算
        pad_w = (self.size - new_w) // 2
        pad_h = (self.size - new_h) // 2
        
        # 黒色（0）でパディングして正方形にする
        img = F.pad(img, (pad_w, pad_h, self.size - new_w - pad_w, self.size - new_h - pad_h), fill=0)
        return img