...
from utility import plot_learning_curve, plot_pr_curve, LetterboxResize
...
def build_transform(image_size: int):
    return transforms.Compose(
        [
            LetterboxResize(image_size), # utility.py からインポートしたツールを使用
            transforms.ToTensor(),
...
def main() -> None:
    ...
    # 常に重み付けを行う
    criterion = build_criterion(train_df, GENRE_COLS, device)
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        if row["val_loss"] < best_val_loss:
            ...
            # 生データを取り出してPR曲線を更新
            plot_pr_curve(val_metrics["_raw_probs"], val_metrics["_raw_targets"], output_dir)
            print(f"Updated PR curve at {output_dir / 'pr_curve_best.png'}")
