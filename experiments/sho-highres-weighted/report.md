...
| 項目 | 内容 |
|---|---|
| 実験ID | sho-highres-weighted (実験C: 統合実験) |
| 日付 | 2026-06-25 |
| 担当者 | sho |
| タスク | アニメのキービジュアルからジャンルを推定するマルチラベル分類 |
| 旧モデル | sho-high-res (実験B: 高解像度・標準Loss) |
| 新モデル | sho-highres-weighted (384x384, LetterboxResize, pos_weightあり) |
| 今回の目的 | 実験Bで向上した「視力」と、実験Aの「少数派へのペナルティ」を統合し、両者がシナジーを生むか、過学習が完全に暴走するかの限界を検証する。 |
| Git branch |  |
| Git commit |  |
| 実験ディレクトリ | `experiments/sho-highres-weighted` |
| config | `experiments/sho-highres-weighted/config.yaml` |
| metrics | `experiments/sho-highres-weighted/outputs/metrics.csv` |
| checkpoint | `experiments/sho-highres-weighted/outputs/best_model.pth` |
...
```markdown
- 解像度を上げたことで基礎的な視力（Micro PR曲線）は改善したが、ペナルティ（重み付け）がないため、依然として少数派ジャンル（Thriller等）の予測精度が低い。
- 増大した情報量を「ノイズの丸暗記」に使ってしまい、Train Lossが消失する過学習が起きている。
```
...
```markdown
- 少数派ジャンルを見逃してしまう不均衡問題に対して、高解像度の情報を用いた再アプローチ。
```
...
```markdown
「情報不足（低解像度）」という足枷が外れた状態であれば、モデルが重み付けのペナルティにパニックを起こさず、正しく少数派の特徴を見つけ出せるようになるかを検証するため。
```
...
| H1 | 実験Aでの学習の崩壊（ジグザグ） | 実験Aでパニックが起きたのは、情報が足りない（224x224の歪んだ画像）のに強いペナルティを与えたからである。 | 実験Bで解像度を上げた結果、地力（Micro PR）が向上した事実。 | 実験Bの高解像度環境下で、再度 `pos_weight` を適用して学習曲線を観察する。 |
...
```markdown
- 実験B（`sho-high-res`）の環境をベースに、損失関数を `criterion.py` にて `pos_weight` を動的計算する BCEWithLogitsLoss に変更した（実験Aの力学の再適用）。
```
