...
def run_single_seed(config_path: Path, base_config: dict, seed: int, multi_seed: bool) -> dict:
    ...
    writer = None
    if bool(config["use_tensorboard"]):
        writer = SummaryWriter(log_dir=output_dir / f"tensorboard/{time.strftime('%Y%m%d-%H%M%S')}")
    ...
    train_loader = DataLoader(
        ...
        persistent_workers=True if int(config["num_workers"]) > 0 else False,
    )
    ...
    ema_model = AveragedModel(
        model=model,
        multi_avg_fn=get_ema_multi_avg_fn(decay=float(config.get("ema_decay"))),
        use_buffers=True,
        device=device
    )
    ...
    scheduler = build_scheduler(optimizer, steps_per_epoch=steps_per_epoch, epochs=int(config["epochs"]), max_lr=float(config["learning_rate"]))

    print(
        "Gradient accumulation: "
        f"{gradient_accumulation_steps} step(s) "
        f"(effective batch size: {int(config['batch_size']) * gradient_accumulation_steps})"
    )
    if amp_enabled:
        print("AMP: enabled")
    elif amp_requested:
        print(f"AMP: disabled (CUDA required; current device: {device.type})")
    else:
        print("AMP: disabled by config")
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        if metric_improved(monitor_value, best_monitor_value, mode, min_delta):
            ...
            save_checkpoint(ema_model.module, checkpoint_path)
