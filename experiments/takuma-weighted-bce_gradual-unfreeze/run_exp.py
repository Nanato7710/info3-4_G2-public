...
def run_single_seed(config_path: Path, base_config: dict, seed: int, multi_seed: bool) -> dict:
    ...
    # 初期時点（ヘッドのみ学習対象）のオプティマイザを作成
    optimizer = build_optimizer(model, float(config["learning_rate"]))
    
    # 🌟 【新機能】現在の学習対象のパラメータ数を初期記録
    prev_trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        # 1. 段階的解凍スケジュールに合わせて、モデル側のrequires_gradを切り替える
        raw_model = getattr(model, "_orig_mod", model)  # torch.compile対策
        raw_model.unfreeze_stage(epoch)
        
        # 2. 🌟 【自動検知】現在の学習対象パラメータ数をチェック
        current_trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        
        # 3. 🌟 【自動検知】パラメータ数に変化があった場合（＝新しいレイヤーが解凍されたステージ移行時）
        if current_trainable_params != prev_trainable_params:
            print(f"\n[Gradual-Unfreeze] 🚀 Stage Changed at Epoch {epoch}!")
            print(f"Trainable parameters increased: {prev_trainable_params} -> {current_trainable_params}")
            
            # 安全にオプティマイザを再生成（内部ステートのエラーを完全に防ぐ）
            optimizer = build_optimizer(model, float(config["learning_rate"]))
            
            # 前のステージまでの停滞カウンタをリセット（即死バグを完全に防止！）
            epochs_without_improvement = 0
            print("Optimizer re-initialized and Early Stopping counter reset safely.\n")
            
            # 次回比較用にパラメータ数を更新
            prev_trainable_params = current_trainable_params
